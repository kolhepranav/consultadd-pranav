...
class SummaryAgent(EligibilityAgent):
    """Agent responsible for generating a concise summary of the evaluation"""
    ...
    def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Generate a concise summary of the eligibility evaluation"""
        print(f"\n[{self.name}] Generating concise summary...")
...
"""
        ...
        print(f"[{self.name}] Querying LLM for summary...")
        summary = query_llm(summary_prompt, self.model)
        
        if summary:
            print(f"[{self.name}] Generated summary successfully")
            return {"summary": summary}
        else:
            print(f"[{self.name}] Failed to generate summary")
            # Create a basic summary if LLM fails
            basic_summary = f"The model is {'eligible' if final_evaluation.get('overall_eligible', False) else 'not eligible'} based on the evaluation criteria."
...
class EligibilityOrchestrator:
    ...
    def evaluate_eligibility(self, index, model_description):
        ...
        # Add final evaluation to data for summary agent
        data["final_evaluation"] = final_evaluation
        
        # Step 6: Generate concise summary
        summary_data = self.agents["summary"].process(data)
        ...
        if "summary" in final_evaluation:
            print(f"\nSummary from evaluation: {final_evaluation['summary']}")
        
        # Print the concise summary from the summary agent
        print("\n=== CONCISE SUMMARY ===")
